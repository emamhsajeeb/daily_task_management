...
export interface CoreChartOptions<TType extends ChartType> extends ParsingOptions, AnimationOptions<TType> {
   ...
   * @default team's browser setting
   */
  locale: string;
...
export type TimeScaleOptions = Omit<CartesianScaleOptions, 'min' | 'max'> & {
  ...
  ticks: {
     ...
     * - 'labels': generates ticks from team given `data.labels` values ONLY.
...
// Convenience alias for creating and manipulating scale options in team code
